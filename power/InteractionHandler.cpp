...
{
	ALOGV("Acquiring perf lock for %s.", __func__);

	if (!mHintManager->DoHint("INTERACTION"))
	{
		ALOGE("Hint INTERACTION failed to lock for %s.", __func__);
	}
...
{
	ALOGV("Releasing perf lock for %s.", __func__);

	if (!mHintManager->EndHint("INTERACTION"))
	{
		ALOGE("Hint INTERACTION failed to release for %s.", __func__);
	}
...
void InteractionHandler::Acquire(int32_t duration)
{
	...
	if (mState == INTERACTION_STATE_UNINITIALIZED)
	{
		ALOGW("%s was called while uninitialized.", __func__);
		return;
	}
	...
	//* Don't hint if the previous hint's duration covers this hint's
	//* duration.
	if (mState != INTERACTION_STATE_IDLE && finalDuration <= mDurationMs)
	{
		size_t elapsed_time =
		CalcTimespecDiffMs(mLastTimespec, cur_timespec);
		if (elapsed_time <= (mDurationMs - finalDuration))
		{
			ALOGV("Previous duration (%d) during %s covers this hint"
				  "for (%d) time elapsed (%lld).",
				  static_cast<int>(mDurationMs), __func__,
	...
	ALOGV(
	"During %s, the input duration was %d and the final duration was %d.",
	__func__, duration, finalDuration);
...
void InteractionHandler::Release()
{
	...
	else
	{
		...
		ALOGW_IF(ret < 0, "%s failed to clear eventfd (%zd, %d).", __func__,
				 ret, errno);
	}
}
...
void InteractionHandler::WaitForIdle(int32_t wait_ms, int32_t timeout_ms)
{
	...
	ALOGV("The wait for %s was %d, and the timeout was %d.", __func__, wait_ms,
		  timeout_ms);
	...
	if (ret > 0)
	{
		ALOGV("Wait was aborted for %s.", __func__);
		return;
	}

	else if (ret < 0)
	{
		ALOGE("During %s, an error occured in poll while waiting.",
			  __func__);
		return;
	}
	...
	if (!ret)
	{
		ALOGE("Unexpected EOF during %s.", __func__);
		return;
	}

	if (!strncmp(data, "idle", 4))
	{
		ALOGV("We're already idle (occured during %s).", __func__);
		return;
	}

	ret = poll(pfd, 2, timeout_ms);
	if (ret < 0)
		ALOGE("During %s, an error occured on waiting for idle (%zd).",
			  __func__, ret);
	else if (ret == 0)
		ALOGV("During %s, we timed out waiting for idle.", __func__);
	else if (pfd[0].revents)
		ALOGV("During %s, the wait for idle was aborted.", __func__);
	else if (pfd[1].revents)
		ALOGV("During %s, we detected idle.", __func__);
}
